  ...
  Future<void> fetchClasses() async {
    ...
    try {
      ...
      final response = await http.get(
        Uri.parse('http://localhost:8000/api/v4/routines/ongoing?group=$group'),
        headers: {
...
//       if (response.statusCode == 200) {
//         List<dynamic> data = json.decode(response.body)['ongoingClassGroup/s'];  // Fix the key name here
//         classes.value = data.map((e) => ClassModel.fromJson(e)).toList();  // Parse data into ClassModel
//       } else {
//         print('Failed to load data. Status code: ${response.statusCode}');
//       }
//     } catch (e) {
...
//   // Get yesterday's date label
//   String getYesterday() {
//     return DateFormat('EEEE').format(DateTime.now().subtract(Duration(days: 1)));
//   }
...
//   List<String> getDaysAfterTomorrow() {
//     return List.generate(5, (index) {
//       return DateFormat('EEEE').format(DateTime.now().add(Duration(days: index + 2)));
//     });
//   }
// }
